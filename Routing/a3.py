...
def install_udp_middlebox_flow( source_dpid
    ...
    network = get_networkx_topology_graph()

    # get the shortest path to and from the middle box
    src_middlebox = get_shortest_path_between(network, source_dpid, middlebox_dpid)
    middlebox_dst = get_shortest_path_between(network, middlebox_dpid, destination_dpid)

    # get necessary identification values
    srcip = get_host_ip_from_dpid(source_dpid)
    srcmac = get_host_mac_from_dpid(source_dpid)
    dstip = get_host_ip_from_dpid(destination_dpid)
    dstmac = get_host_mac_from_dpid(destination_dpid)

    # source to middle box
    for i in range(len(src_middlebox)):
        curr_switch = src_middlebox[i]
        inport = get_input_port(src_middlebox, curr_switch)
        match = build_match_for(srcmac, dstmac, srcip, dstip, source_port, destination_port, inport)
    
        # for switches before the middle box
        if i < len(src_middlebox) - 1:
            port = get_ports_connecting(curr_switch, src_middlebox[i+1])[0]
        # middle box
        else:
            port = get_host_port(curr_switch)

        # install the flow
        flow = build_openflow_flowmod(match, port)
        install_flowmod_on_switch_with_dpid(flow, curr_switch)

    # middle box to destination
    for i in range(len(middlebox_dst)):
        curr_switch = middlebox_dst[i]
        inport = get_input_port(middlebox_dst, curr_switch)
        match = build_match_for(srcmac, dstmac, srcip, dstip, source_port, destination_port, inport)

        # for switches before the destination
        if i < len(middlebox_dst) - 1:
            port = get_ports_connecting(curr_switch, middlebox_dst[i+1])[0]
        # destination
        else:
            port = get_host_port(curr_switch)
